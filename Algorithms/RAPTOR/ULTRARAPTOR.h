...
template<bool TARGET_PRUNING, typename INITIAL_TRANSFERS = BucketCHInitialTransfers, typename PROFILER = NoProfiler, bool USE_MIN_TRANSFER_TIMES = false, bool PREVENT_DIRECT_WALKING = false>
class ULTRARAPTOR {
...
public:
    ...
    using Type = ULTRARAPTOR<TargetPruning, InitialTransferType, Profiler, UseMinTransferTimes, PreventDirectWalking>;
...
private:
    ...
    inline void getJourney(std::vector<Journey>& journeys, size_t round, StopId stop) const noexcept {
        ...
        do {
            ...
            if (!journey.empty() && !label.usesRoute && !journey.back().usesRoute) {
                journey.back().from = label.parent;
                journey.back().departureTime = label.parentDepartureTime;
