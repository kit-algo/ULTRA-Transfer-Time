...
#include "../Query/ReachedIndexSmall.h"
...
public:
    ...
    using Type = ForwardPruningQuery<Debug>;
...
public:
    ForwardPruningQuery(const Data& data, CH::BucketQuery<CHGraph, true, false>& bucketQuery, BoundedProfiler& profiler) :
        data(data),
...
private:
    ...
    inline void computeInitialAndFinalTransfers() noexcept {
        if constexpr (Debug) profiler.chTimer.restart();
        ...
        if constexpr (Debug) profiler.chTime += profiler.chTimer.elapsedMicroseconds();
    }
...
private:
    ...
    size_t queueSize;
    ReachedIndexSmall reachedIndex;
