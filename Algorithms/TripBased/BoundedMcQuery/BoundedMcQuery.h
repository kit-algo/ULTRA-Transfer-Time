...
public:
    ...
    using Type = BoundedMcQuery<Debug>;
...
public:
    ...
    inline void run(const Vertex source, const int departureTime, const Vertex target, const double arrivalSlack, const double tripSlack) noexcept {
        ...
        clear();
        if constexpr (Debug) profiler.clearTime += profiler.phaseTimer.elapsedMicroseconds();
        sourceVertex = source;
        targetVertex = target;
        ...
        if constexpr (Debug) profiler.forwardTime += profiler.phaseTimer.elapsedMicroseconds();
        if (forwardPruningQuery.getAnchorLabels().empty()) return;
        maxTrips = forwardPruningQuery.getMaxTrips();
        if constexpr (Debug) profiler.roundCount += maxTrips;
        if constexpr (Debug) profiler.phaseTimer.restart();
        backwardPruningQuery.run(target, departureTime, source, arrivalSlack, tripSlack);
        if constexpr (Debug) profiler.backwardTime += profiler.phaseTimer.elapsedMicroseconds();

        if constexpr (Debug) profiler.phaseTimer.restart();
        computeInitialAndFinalTransfers();
        evaluateInitialTransfers();
        scanTrips();
        if constexpr (Debug) profiler.mainTime += profiler.phaseTimer.elapsedMicroseconds();
    ...
    inline BoundedProfiler& getProfiler() noexcept {
        return profiler;
    }
...
private:
    ...
    ForwardPruningQuery<Debug> forwardPruningQuery;
    BackwardPruningQuery<Debug> backwardPruningQuery;
