...
public:
    ...
    public:
        ...
        inline bool prune(const Label& newLabel) noexcept {
            return internalMerge<false>(newLabel, [&](const Label& label) {
        ...
        inline std::tuple<bool, bool> mergeWithCandidateDomination(const Label& newLabel, const FUNCTION& processRemovedHeapLabel = NoOperation) noexcept {
            bool isProper = true;
...
private:
    ...
    inline void scanRoutes1(const bool forCandidates) noexcept {
        ...
        for (const RouteId route : routesServingUpdatedStops.getKeys()) {
            ...
            while (tripIterator.hasFurtherStops()) {
                ...
                tripIterator.nextStop();
                const OneTripLabel label(tripIterator.arrivalTime(), walkingDistance, forCandidates ? StopEventId(tripIterator.stopEvent() - &(data.stopEvents[0])) : noStopEvent, timestamp);
        ...
        for (const RouteId route : routesServingUpdatedStops.getKeys()) {
            ...
            while (stopIndex < tripSize - 1) {
                ...
                for (const RouteLabel& label : routeBag) {
                    TwoTripsLabel newLabel(label.trip[stopIndex].arrivalTime, label.walkingDistance, noStopEvent, timestamp);
    ...
    inline void arrivalByRoute2(const StopId stop, TwoTripsLabel& label, const RouteLabel& routeLabel, const StopEventId finalStopEvent) noexcept {
        ...
        std::tie(wasInserted, isProper) = updateTwoTripsBagWithCandidateDomination(stop, label);
    ...
    inline void arrivalByEdge0(const Vertex vertex, const int walkingDistance) noexcept {
        const int arrivalTime = walkingDistance + sourceDepartureTime;
        if (oneTripBags[vertex].prune(OneTripLabel(arrivalTime, walkingDistance, noStopEvent, -1))) {
            pruneBagInQueue(oneTripBags[vertex], oneTripQueue);
        }
        if (twoTripsBags[vertex].prune(TwoTripsLabel(arrivalTime, walkingDistance))) {
    ...
    inline std::tuple<bool, bool> updateTwoTripsBagWithCandidateDomination(const Vertex vertex, const TwoTripsLabel& newLabel) noexcept {
        const OneTripLabel walkingLabel = getWalkingLabel(vertex);
        ...
        refreshTwoTripsBag(vertex, walkingLabel);
        return twoTripsBags[vertex].mergeWithCandidateDomination(newLabel, [&](const TwoTripsLabel& removedLabel) {
            pruneShortcutDestinationCandidates(removedLabel);
        });
    }
